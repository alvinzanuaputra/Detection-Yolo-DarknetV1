...
Darknet adalah framework neural network open source yang ditulis menggunakan C dan CUDA. Framework ini cepat, mudah diinstall, dan mendukung komputasi CPU maupun GPU.
...
*   **OS**: Linux (Ubuntu/Debian sangat disarankan) atau Windows (dengan WSL atau build khusus).
*   **Compiler**: `gcc` atau `clang`.
*   **CUDA** (Opsional, tapi sangat disarankan untuk performa): Jika Anda memiliki GPU NVIDIA.
*   **OpenCV** (Opsional): Untuk menampilkan hasil deteksi secara visual dan memproses video.
...
Sebelum melakukan kompilasi, Anda perlu mengedit file `Makefile` untuk mengaktifkan fitur GPU dan OpenCV.

Buka file `Makefile` dengan text editor (seperti `nano` atau VS Code) dan ubah baris berikut di bagian paling atas:

```makefile
GPU=1      # Ubah menjadi 1 jika menggunakan NVIDIA GPU dan CUDA
CUDNN=1    # Ubah menjadi 1 jika menggunakan cuDNN (untuk percepatan lebih lanjut)
OPENCV=1   # Ubah menjadi 1 untuk dukungan video dan webcam
OPENMP=0   # (Opsional) Ubah menjadi 1 untuk penggunaan multi-core CPU
DEBUG=0
```
...
Keduanya opsional, jadi bisa dimulai dengan menginstal sistem dasarnya saja. Saya hanya menguji ini di komputer Linux.
...
```
(Pastikan `OPENCV=1` di Makefile dan webcam terdeteksi di `/dev/video0`).
